...
local function validate_progress_report(action, step_names)
  ...
  local opts = { kind = 'progress', title = 'vim.pack', status = 'running', percent = 0 }
  local init_step = { { { ('%s (0/%d)'):format(action, n_steps) } }, true, opts }
  ...
  for i = 1, n_steps do
    ...
    opts.id = opts.id or echo_args[3].id ---@type integer
    opts.percent = math.floor(100 * i / n_steps)
    eq(echo_args[3], opts)
  end
  ...
  opts.percent, opts.status = 100, 'success'
  local final_step = { { { ('%s (%d/%d)'):format(action, n_steps, n_steps) } }, true, opts }
