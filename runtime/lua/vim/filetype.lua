...
-- - Match at least the same set of strings filetype patterns inside it. But not too much more.
...
-- - If there is a group with appropriate fast pattern, use it.
...
--
-- When modifying an existing regular pattern, make sure that it still fits the right group.
--
...
local function sort_by_priority(t)
  ...
  for fastpat, ft_map in pairs(t) do
    for pat, v in pairs(ft_map) do
...
M.get_info = function()
  return pattern_sorted_pos, pattern_sorted_neg
end
...
function M.add(filetypes)
  ...
  for k, v in pairs(filetypes.pattern or {}) do
    -- Add to "match all" parent fast pattern (might be better to optimize or allow supplying
    -- `opts.fastpat` directly)
