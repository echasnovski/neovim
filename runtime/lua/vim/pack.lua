...
local function new_progress_report(action)
  local echo_opts = { kind = 'progress', title = 'vim.pack' }

  return vim.schedule_wrap(function(kind, percent, fmt, ...)
    echo_opts.status = kind == 'end' and 'success' or 'running'
    echo_opts.percent = percent
    local details = ('%s %s'):format(action, fmt:format(...))
    echo_opts.id = api.nvim_echo({ { details } }, kind ~= 'report', echo_opts)
